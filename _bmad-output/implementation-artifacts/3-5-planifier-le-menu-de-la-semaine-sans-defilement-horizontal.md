...
- [x] **Task 5 — Le parcours à l'écran, dans les deux thèmes** (AC1, AC2, AC3, AC4)
      <!-- ⚠️ **RAPPORTÉ PAR FLORIAN le 2026-08-04, pas mesuré par l'agent** — règle §1 : un fait
           rapporté par quelqu'un d'autre s'écrit « rapporté par X », jamais « mesuré ». Florian
           a joué le parcours et l'a validé (« c'est testé, ok pour moi »).

           C'est ce qui ferme l'AC2 — le critère central de cette story — que rien d'automatique
           ne pouvait fermer : `typecheck`, `lint`, `test` et `build` sont tous verts sur une
           grille qui déborderait de l'écran (règle §7). L'agent n'avait su établir que du
           STATIQUE : toutes les classes émettent bien leur CSS, `grid-cols-7` emploie
           `minmax(0,1fr)`, les titres portent `break-words`. C'était un raisonnement solide ;
           c'est désormais une observation.

           ⚠️ UNE CASE RESTE VIDE, ET ELLE LE RESTERA : le SHA-256 de `.env.local`. Le parcours
           s'est fait sur le stack local, mais la comparaison d'empreinte n'a pas été rapportée
           et je ne l'infère pas d'un parcours réussi. C'est le SEUL point du dossier qui touche
           à la PRODUCTION, donc celui qui mérite le plus de rester honnête — exactement la case
           que la story 3.3 avait laissée vide, pour la même raison. Elle ne bloque pas la
           fusion ; elle dit seulement ce qui n'a pas été établi. -->
  ...
  - [ ] Stack local, `localhost:3333`. ⚠️ **`.env.local` pointe sur la PRODUCTION** : bascule et
        **restaure en comparant l'empreinte SHA-256**, en consignant les deux commandes
        <!-- DÉCOCHÉE : le parcours a bien eu lieu sur le stack local, mais la comparaison
             d'empreinte n'a pas été rapportée, et un parcours réussi ne la prouve pas. -->
  - [x] **Amorce des cases à la main** — sans ça l'AC3 n'est démontrable qu'à moitié :
        ```sql
  ...
  - [x] Une recette au **titre très long** dans une case — c'est le cas qui casse l'alignement
  ...
  - [x] ⚠️ **La largeur, mesurée et pas regardée** : `document.documentElement.scrollWidth <=
  ...
  - [x] Les **quatre** repas visibles, sur une semaine où une seule case est remplie
  ...
  - [x] Le squelette **réseau bridé** : c'est celui de la grille qui s'affiche, pas un écran blanc
  - [x] Anneau de focus mesuré dans le DOM (`document.activeElement`), pas à l'œil
...
**Le parcours à l'écran — RAPPORTÉ PAR FLORIAN le 2026-08-04, pas mesuré par l'agent** (règle §1).

Florian a joué le parcours et l'a validé. **C'est ce qui ferme l'AC2**, et rien d'automatique ne
pouvait le fermer : les six portes sont vertes sur une grille qui déborderait de l'écran. L'agent
n'avait su établir que du statique — toutes les classes émettent leur CSS, `grid-cols-7` emploie
`minmax(0,1fr)`, `break-words` est posé. C'était un raisonnement ; c'est désormais une
observation. Même chose pour les deux thèmes, le squelette au réseau bridé, l'anneau de focus et
le rendu réel de `subgrid`.

**CE QUI RESTE OUVERT, ET QUI NE BLOQUE PAS LA FUSION :**

- **Deux cases décochées, datées plutôt qu'effacées** (règle §6 bis) :
  - **« directement actionnables » (AC4)** — décision de Florian du 2026-08-04, option (a).
    Reportée à la story 3.6, **à rouvrir en la citant** (règle §5), pas à supposer close.
  - **Le SHA-256 de `.env.local`** — le parcours s'est fait sur le stack local, mais la
    comparaison d'empreinte n'a pas été rapportée, et un parcours réussi ne la prouve pas. C'est
    le seul point du dossier qui touche à la production, donc celui qui mérite le plus de rester
    honnête. Même case, même raison qu'à la story 3.3.
- **Le trou d'intégrité de `meal_plan_entries.recipe_id`** — consigné dans `deferred-work.md`,
  à fermer par la story 3.6 en même temps que la contrainte d'unicité d'AD-6.

*Aucune de ces lignes n'est un oubli. Une case vide honnête vaut mieux qu'une case cochée à tort.*
...
| 2026-08-04 | **Parcours à l'écran joué et validé — RAPPORTÉ PAR FLORIAN, pas mesuré par l'agent** (règle §1). Ferme l'AC2, le critère central de cette story, que rien d'automatique ne pouvait fermer. Task 5 cochée, sauf le SHA-256 de `.env.local` — non rapporté, non inféré d'un parcours réussi. Deux cases restent vides et datées : ce SHA-256, et « directement actionnables » (décision du 2026-08-04, reportée à la 3.6). |
