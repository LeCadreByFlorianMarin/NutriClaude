...
**AC3 — Une résurrection n'a lieu que si l'intention est plus récente**

**Given** un article précédemment supprimé (tombstoné) que la génération réclame
**When** la génération s'exécute
**Then** il n'est **ressuscité que si l'intention de génération est plus récente que l'intention de
suppression** (LWW sur `deleted_at`, AD-3)
...
- [x] **Task 2 — La migration** (AC: 1, 2, 3) · *dépend de D1, D2, D5, D6*
  ...
  - [x] ⛔ **Retirer `added_by`** (D6) — `drop column`, avec sa requête de contrôle exécutée
...
- [x] **Task 4 — L'écran** (AC: 2)
  ...
  - [x] ⚠️ **La liste se RELIT après génération** : on ne peut pas deviner ce qu'elle a fait (agrégé ? rouvert ? ignoré ?)
